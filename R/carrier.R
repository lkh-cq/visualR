...
#'   transformation (121 cells = layer diffusion). Cell value is the
#'   order (F=0 at center, increasing outward to A=5 at edges),
#'   following the frozen mixed Manhattan/Chebyshev rule:
#'   normal zone min(i+j,5); square zone max(i,j); pure diagonal
#'   2*ceil(i/2); far-corner min(i+j,5).
#' @param as_symbol logical, return order symbols (F..A) instead of
#'   integer orders. Default TRUE.
#' @return 11x11 matrix: integer orders (0..5) or character symbols.
#' @examples
#' carrier_11x11()
#' carrier_11x11(as_symbol = FALSE)
carrier_11x11 <- function(as_symbol = TRUE) {
